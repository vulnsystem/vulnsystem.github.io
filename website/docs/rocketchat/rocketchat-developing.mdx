...
yarn
```
2. It will take long time to build porject with yarn dsv command.
```bash
yarn dsv # run only meteor (front and back) with pre-built packages
```
3. If server runing info showed in the shell, it means build and run the RocketChat Server successfully.
