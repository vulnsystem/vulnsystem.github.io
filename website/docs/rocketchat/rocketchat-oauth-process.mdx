...
sequenceDiagram
  ...
  Oauth2Authorize ->> RocketChatServer: Request to redirect_uri with code and state
  ...
  RocketChatServer ->> RocketChatServer: Collection Identity Info

  RocketChatClient ->>+ RocketChatServer: Login using authentication providers
  RocketChatServer -->> RocketChatClient: Resource response
  Note right of RocketChatServer: auth providers
...
## Generate credentialToken then request authorization code
...
## Request to redirect_uri with code and state
Because the LoginUrl contain the redirect_uri as parameter, the Oauth2 Authorize will request to redirect_uri after get auth code. In the following, the redirect_uri with code, state, and session_state as parameter to obtain access_token from Oauth2 token service in the Authorizaiton platform.
